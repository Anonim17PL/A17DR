	...
	if (ftp_enabled == "1") {
		...
		if (curl) {
			...
			if (res != CURLE_OK) {
				...
				errorcode = MessageBox(NULL, ("Failed to send csv to FTP server!\n"+string(error)).c_str(), (string(REMOTE_URL) + " - FTP Client").c_str(), MB_RETRYCANCEL+MB_ICONERROR);
			} else
				errorcode = MessageBox(NULL, "Successfully uploaded csv file to FTP server!", (string(REMOTE_URL) + " - FTP Client").c_str(), MB_OK);
...
extern "C" {
	...
	__declspec(dllexport)void __stdcall PluginStart(void* aOwner)
	{
		...
		MessageBox(NULL,aaa.c_str(), ("Hello " + ftp_username + "!" + " - FTP Client").c_str(), MB_OK);
